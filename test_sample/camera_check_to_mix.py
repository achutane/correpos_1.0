...
class Main(QWidget):
	...
	def initUI(self):  
		...
		self.button2.clicked.connect(self.flagcheck)
	...
	def paintEvent(self,event):
		...
		if(self.flag == 1):
			if self.width >= self.width_s*1.5 and self.height >= self.height_s*1.5:
				self.check = self.check + 1
				print(self.check)

		if(self.flag == 1 and self.check == 100):
			print('tututututututu')
			self.play_kenti("kenti",100)
			self.check = 0
	...
	def buttonClicked(self):
		...
		self.play_shot("shot",50)
		self.width_s = self.width
		self.height_s = self.height
		print(self.width_s)
		print(self.height_s)
